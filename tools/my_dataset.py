...
class AVDataset(Dataset):
    def __init__(self, spvType, dataList):
        """
        mmtracker的Dataset

        """
        self.dataList  = dataList
        self.spvType = spvType

    def __getitem__(self, index):
        dataPath= self.dataList[index]
        data = np.load(dataPath)
        avdata= data['data']
        if self.spvType == 'unspv':
            label = data['labelUnspv']
        elif self.spvType == 'inspv':
            label = data['labelSpv']
        imgPath = str(data['imgPath'])
        gt = data['gt']
        frame = data['frame']

        return avdata, label, imgPath, gt, frame

    def __len__(self):
        return len(self.dataList)#len(dataset) return the range of random index
